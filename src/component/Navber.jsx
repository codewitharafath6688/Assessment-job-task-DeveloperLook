...
const data = [
  ...
  },
  {
    ...
    list: (
      <>
        <li className="text-2xl">B2B Marketing</li>
      </>
    ),
  },
  {
...
const Navber = () => {
  ...
  return (
    <div className="w-full max-w-[1300px] mx-auto -mt-9 rounded-3xl overflow-visible relative text-white">
      {/* 🔥 Blurry Background */}
      <div
        className="absolute inset-0 bg-cover bg-center blur-[8px] scale-110 rounded-3xl"
        style={{
          backgroundImage:
      ...
      {/* Content */}
      <div className="relative w-full z-10 px-3 sm:px-4 md:px-6 py-2">
        {/* Navbar Row */}
        <div className="flex px-2 rounded-2xl w-full justify-between items-center h-[68px] mx-auto">
          {/* Logo */}
          <div className="w-[120px] sm:w-[150px] md:w-[170px] flex-shrink-0">
            <svg
          ...
          <nav className="hidden lg:block">
            <ul className="flex justify-center items-center gap-3 text-sm">
              {data.map((dat, i) => (
                <NavLinkList key={i} content1={dat.name} content2={dat.list} />
              ))}
              <li className="cursor-pointer p-2 hover:bg-zinc-700 rounded-2xl">
                Work
              </li>
              <li className="cursor-pointer p-2 hover:bg-zinc-700 rounded-2xl">
                Careers
              ...
              <NavLinkList
                content1={"Blog & Resources +"}
                content2={
                  <>
                    <li>Blog</li>
                    <li>Category Leaderboard</li>
          ...
          <div className="hidden lg:block">
            <Button1 content={"Get In Touch"} />
          </div>

          {/* Hamburger Button (mobile/tablet) */}
          <button
            className="lg:hidden flex flex-col justify-center items-center gap-1.5 p-2 rounded-xl hover:bg-zinc-700 transition-colors"
        ...
        {menuOpen && (
          <div className="lg:hidden bg-black/70 backdrop-blur-sm rounded-2xl mt-2 px-4 py-4 flex flex-col gap-3 text-sm">
            {data.map((dat, i) => (
              <button
                key={i}
                className="text-left px-3 py-2 hover:bg-zinc-700 rounded-xl transition-colors"
              >
                {dat.name}
              </button>
            ))}
            <button className="text-left px-3 py-2 hover:bg-zinc-700 rounded-xl">Work</button>
            <button className="text-left px-3 py-2 hover:bg-zinc-700 rounded-xl">Careers</button>
            <button className="text-left px-3 py-2 hover:bg-zinc-700 rounded-xl">Blog &amp; Resources +</button>
            <button className="text-left px-3 py-2 hover:bg-zinc-700 rounded-xl">Webinar</button>
            <div className="pt-2">
              <Button1 content={"Get In Touch"} />
            </div>
          </div>
        ...
        {/* Hero Content */}
        <div className="mt-8 sm:mt-10 md:mt-13 text-center px-2 sm:px-4">
          <div className="flex flex-col justify-center items-center gap-5">
            <h3 className="font-medium text-xs sm:text-sm tracking-wide">
              #1 MOST RECOMMENDED <br />
              CONTENT MARKETING AGENCY
            </h3>
            <Performance />
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold leading-tight mt-6">
            We Create <br />
            <div className="flex justify-center gap-3 sm:gap-5 items-center flex-wrap">
              Category {/* 🔥 Image Block inside text */}
              <div className="w-[50px] sm:w-[65px] md:w-[80px]">
                <img
                  className="w-full h-[60px] sm:h-[75px] md:h-[90px] rounded-2xl object-cover object-bottom"
          ...
          <h2 className="mt-4 text-base sm:text-lg md:text-[24px] font-bold text-gray-300">
            on every searchable platform
          </h2>
        </div>

        {/* Bottom Text */}
        <div className="flex flex-col md:flex-row justify-between mt-5 text-xs sm:text-sm text-gray-300 gap-4 px-2 sm:px-4 pb-4">
          <h3>
